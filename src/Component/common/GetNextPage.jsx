...
const Main = (mySetting) => {
        ...
        render() {
            ...
            return (
                <div>
                    ...
                    <div ref="dataload" className="wy" style={{background: 'red'}}><DataLoad loadAnimation={loadAnimation} loadMsg={loadMsg} /></div>
                </div>
            );
        }
        /**
    ...
    return connect((state) => {console.log(state); return { state: state[setting.id], User: state.User } }, action(action.id))(Index); //连接redux
}
